...
int GUIAction::flash(std::string arg)
{
	...
	if (DataManager::GetIntValue(PB_INSTALL_PREBUILT_ZIP) != 1 && DataManager::GetIntValue(PB_DISABLE_DEFAULT_DM_VERITY) != 1)
	{
...
int GUIAction::flashimage(std::string arg __unused)
{
	...
	if (DataManager::GetIntValue(PB_CALL_DEACTIVATION) != 0 && DataManager::GetIntValue(PB_DISABLE_DEFAULT_DM_VERITY) != 1)
	{
		TWFunc::Deactivation_Process();
	}
	DataManager::SetValue(PB_CALL_DEACTIVATION, 0);
